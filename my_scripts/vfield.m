...
x = reshape(x,1,n);
y = reshape(y,1,n);
if numel(z) == 1, z = repmat(z,size(x)); end
u = reshape(u,1,n);
v = reshape(v,1,n);
speed = sqrt(u.^2 + v.^2);
ang   = atan2(v,u);
Fi    = Fi*pi/180;
if ~ischar(a), r = speed*a;
else          r = repmat(str2double(a),size(speed)); end
...
if ~fillit && ~useC
  theView = view;
  tmpx = [x; x+u; P1x; x+u; P2x; nan(size(x))];
  tmpy = [y; y+v; P1y; y+v; P2y; nan(size(x))];
  tmpz = [z;   z;   z;    z;   z; nan(size(x))];
